...
root = '/home/jmn/dev/Datasets'
task = 'RGBT' # 'RGBT' or 'RGBD'
sufix = '/NJUNLPR' if task == 'RGBD' else ''
network = 1
save_name = f'network_{network}_{task}'
save_path = save_name.replace(':', '_')
save_path = save_path.replace(' ', '_')
best_model_path = f'runs/train/{save_path}/Net_epoch_best.pth'
...
parser.add_argument('--train_root', type=str, default=f'{root}/{task}_dataset/train{sufix}', help='the train images root')
parser.add_argument('--val_root', type=str, default=f'{root}/{task}_dataset/val{sufix}', help='the val images root')
parser.add_argument('--save_path', type=str, default=f'runs/train/{save_path}/', help='the path to save models and logs')
parser.add_argument('--wandb_mode', type=str, default=f'disabled', help='type mode (disabled, offline or online)')
