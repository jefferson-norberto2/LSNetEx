...
for t in  range(20):
    ...
    for index, dataset in enumerate(test_datasets):
        ...
        tp_sum, fp_sum, fn_sum, tn_sum, iou_sum = 0, 0, 0, 0, 0
        ...
        print(f"Time taken for {dataset}, with {test_loader.size} images: {elapsed_time:.2f} seconds")
        ...
        print(f"MAE: {test_mae}, IoU: {test_iou}, Precision: {test_precision}")
    
    median = sum(tensor(times)) / len(times)
    print(f"Average time taken for all datasets: {median:.2f} seconds")
