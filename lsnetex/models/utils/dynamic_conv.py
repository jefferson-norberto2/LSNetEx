import torch
import torch.nn as nn
import torch.nn.functional as F

# Define o Dynamic Convolution Module
class DynamicConv(nn.Module):
    def __init__(self, in_channels, out_channels, ksize):
        super(DynamicConv, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels  # Certifique-se de consistência
        self.ksize = ksize

        # Global Average Pooling
        self.global_avg_pool = nn.AdaptiveAvgPool2d(1)

        # Fully connected para calcular pesos dinâmicos
        self.seq = nn.Sequential(
            nn.Conv2d(in_channels, out_channels * 2, ksize, padding=1),
            nn.ReLU(),
            nn.Conv2d(out_channels*2, out_channels*4, ksize, padding=1),
            nn.ReLU(),
            nn.Conv2d(out_channels * 4, out_channels, kernel_size=1)
        )
        
        # Fully connected para calcular pesos dinâmicos
        self.fc = nn.Sequential(
            nn.Linear(in_channels, out_channels * ksize * ksize),
            nn.Sigmoid()
        )

        # Batch Normalization
        self.norm = nn.BatchNorm2d(out_channels)  # Consistência aqui

    def forward(self, x):
        x = self.seq(x)
        B, C, H, W = x.shape

        # Gera os pesos dinâmicos
        pooled = self.global_avg_pool(x).view(B, C)
        dynamic_weights = self.fc(pooled).view(B, self.out_channels, self.ksize * self.ksize)
        
        # Extração dos patches
        x_unfold = F.unfold(x, kernel_size=self.ksize, padding=self.ksize // 2)
        x_unfold = x_unfold.view(B, C, self.ksize * self.ksize, H, W)

        # Ajuste dos pesos dinâmicos
        dynamic_weights = dynamic_weights.view(B, self.out_channels, self.ksize * self.ksize, 1, 1)
        x_unfold = x_unfold.unsqueeze(1)  # Expande para corresponder aos canais de saída

        # Multiplicação e soma ao longo dos patches
        x_dynamic = torch.sum(x_unfold * dynamic_weights, dim=3)
        x_dynamic = torch.sum(x_dynamic, dim=3)  # Reduz a dimensão dos canais de entrada

        x_dynamic = nn.Conv2d(x_dynamic.shape[1], self.out_channels, kernel_size=1)(x_dynamic)

        # Diagnóstico
        print(f"x_dynamic shape before norm: {x_dynamic.shape}, expected out_channels: {self.out_channels}")

        # Normalização
        x_dynamic = self.norm(x_dynamic)  # Deve ser consistente com out_channels
        return x

# Implementação do BBA com Dynamic Convolutions
def tensor_bound_dynamic(img, ksize, dynamic_conv):
    """
    :param img: tensor, B*C*H*W
    :param ksize: int, tamanho do kernel
    :param dynamic_conv: instância do módulo DynamicConv
    :return: tensor, diferença entre inflação e corrosão
    """
    B, C, H, W = img.shape
    pad_value = int((ksize - 1) // 2)
    
    # Aplica padding na entrada
    img_pad = F.pad(img, pad=[pad_value, pad_value, pad_value, pad_value], mode='constant', value=0)
    
    # Calcula a inflação (máximo local) com convolução dinâmica
    inflation = dynamic_conv(img_pad)
    
    # Calcula a corrosão (mínimo local) com convolução dinâmica invertida
    corrosion = -dynamic_conv(-img_pad)
    
    # Retorna a diferença entre inflação e corrosão
    return inflation - corrosion


# Exemplo de uso
if __name__ == "__main__":
    B, C, H, W = 2, 3, 64, 64  # Dimensões da entrada
    ksize = 3  # Tamanho do kernel
    img = torch.randn(B, C, H, W)  # Entrada simulada

    # Instancia DynamicConv
    dynamic_conv = DynamicConv(in_channels=C, out_channels=C, ksize=ksize)

    # Aplica BBA com convoluções dinâmicas
    result = tensor_bound_dynamic(img, ksize, dynamic_conv)
    print("Resultado:", result.shape)
